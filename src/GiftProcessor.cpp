...
std::vector<MenuOption> get_items_options()
{
	...
	result.push_back({ -1, "[QUIT GIFT MENU]" });
...
int get_item_scroll_position()
{
	...
	RE::UI* ui = RE::UI::GetSingleton();
	if (ui)
		if (const auto menu = ui->GetMenu<RE::GiftMenu>(); menu)
			if (menu->uiMovie)
...
std::pair<bool, std::string> set_item_choice(int id)
{
	...
	if (id == -1)
	{
		quit_menu();
		result.first = true;
		result.second = "[Stopped gift menu]";
		return result;
	}
...
void processor(float dtime)
{
	...
	in_container = ui->IsMenuOpen(RE::GiftMenu::MENU_NAME);
