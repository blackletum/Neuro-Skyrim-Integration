    ...
    std::pair<bool, std::string> walk_to_object_by_index(int index, int interaction)
    {
        ...
        if (MiscThings::is_objects_around_valid())
        {
            ...
            if (object != objects_around->end())
            {
                ...
                auto left_hand_contents = MiscThings::get_hand_contents(false);
                ...
                if (object->second.object && object->second.object == eye_of_magnus && staff_of_magnus && MiscThings::magnus_eye_attack_condition())
                {
                    auto magnus_eye = (RE::TESObjectREFR*)RE::TESObjectREFR::LookupByID(0x25224);
                    if (magnus_eye)
                        interaction = 3; //force attacking
                }
    ...
    bool attack_target(float dtime)
    {
        ...
        */
        ...
        if (target_ref && player_ref)
        {
            ...
            left_is_useless |= MiscThings::has_spell_equipped(false) && !inanimate && !left_is_resurrect && !MiscThings::is_summon_spell(false) && !MiscThings::is_self_healing_spell(false) && !MiscThings::is_offensive_spell(false) && !MiscThings::is_scroll(MiscThings::get_hand_contents(false));
            right_is_useless |= MiscThings::has_spell_equipped(true) && !inanimate && !right_is_resurrect && !MiscThings::is_summon_spell(true) && !MiscThings::is_self_healing_spell(true) && !MiscThings::is_offensive_spell(true) && !MiscThings::is_scroll(MiscThings::get_hand_contents(true));
    ...
    std::string get_success_message()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (target_ref == redirect_force_field2_mage && interaction_after_walk != 3)
                {
                    send_random_context("You walked up to the Magical Force Field. You need to attack it with your Staff of Magnus", false);
                    reset_walker();
                    return "";
                }
        ...
        */
            ...
            if (!RE::UI::GetSingleton()->IsMenuOpen(RE::TweenMenu::MENU_NAME) && !RE::UI::GetSingleton()->IsMenuOpen(RE::LevelUpMenu::MENU_NAME) && !RE::UI::GetSingleton()->IsMenuOpen(RE::StatsMenu::MENU_NAME) && (init_delay || true))
            {
                ...
                if (have_target_to_walk)
                {
                    ...
                    if (magnus_eye && target_ref == magnus_eye && interaction_after_walk == 3)
                    {
                        ignore_raycast = true;
                    }
